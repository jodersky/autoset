...
trait MainApi:
  ...
  /** A function to transform the name of an environment variable into a
    ...
    * overriding it in every call of `read`.
    */
  def defaultEnvKeyReplacer: String => String = s =>
  ...
  /** Read a configuration from various sources and formats.
    ...
    *
    * @param readers
    *   Additional readers for file extensions. See `defaultReaders` in the API
    *   trait.
    *
    * @param env
  ...
  def readConfig(
    ...
    def readfile(file: os.FilePath, reader: formats.FileReader) =
      ...
      if !os.exists(abs) then
        throw ReadException(s"config file $file does not exist.")
      if !os.isFile(abs) then
        throw ReadException(s"config file $file is not a file.")
      ...
      result match
        case formats.FileReader.Result.Error(message, row, col, line) =>
          val pos =
            if row > 0 && col > 0 then s"$file:$row:$col"
            else if row > 0 then s"$file:$row"
            else file.toString
    ...
    for root <- paths do
      ...
      if os.isDir(absRoot) then
        ...
        val reader = allReaders.getOrElse(
          ...
          throw ReadException(
            s"config file ${root} has an unknown format. Allowed file extensions are ${allReaders.keySet.mkString(", ")}"
          )
        )
