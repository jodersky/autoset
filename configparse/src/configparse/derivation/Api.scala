...
trait Api extends configparse.model.MainApi with ReaderApi with StandardReaders with DerivationApi:
  ...
  def read[A](
    ...
    val cfg = readConfig(paths, pwd, readers, env, envPrefix, envKeyReplacer, envBinds, props, propsPrefix, propsBinds, args, config)
