  ...
  case class ProductReader[A](
    ...
    def readImpl(cfg: Config, path: Path): Result[A] =
      ...
      if errors.isEmpty then
        ...
        Result.Error(errors.toSeq*)

  // implicit class Derivable(p: Reader.type)
  extension (p: Reader.type)
    inline def derived[A]: ProductReader[A] = derivedImpl[A].asInstanceOf[ProductReader[A]]
...
object macros:
  ...
  def derivedImpl[Api <: DerivationApi: Type, A: Type](api: Expr[Api])(using qctx: Quotes): Expr[DerivationApi#Reader[A]] =
    import qctx.reflect.*
      ...
      for params <- constructor.paramSymss yield
        for param <- params yield
          ...
          Implicits.search(readerType) match
            case iss: ImplicitSearchSuccess => iss.tree.asExprOf[DerivationApi#Reader[?]]
            case _ =>
              report.error(s"No ${api.asTerm.tpe.widenTermRefByName.show}.Reader[${param.termRef.widenTermRefByName.show}] available for parameter ${param.name}.", param.pos.get)
              '{???}
        end for
      end for
    ...
    '{
      ...
      val readers: Seq[DerivationApi#Reader[?]] = ${Expr.ofSeq(fieldReaders.flatten)}
      p.ProductReader(
        names,
        defaults,
